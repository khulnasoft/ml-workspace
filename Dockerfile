FROM ubuntu:20.04 AS workspace-base

USER root

ARG ARG_NB_USER="khulnasoft"
ARG ARG_NB_UID="1000"
ARG ARG_NB_GID="100"
...
ENV \
    ...
    NB_UID=$ARG_NB_UID \
    NB_GID=$ARG_NB_GID \
    HOME="/home/$ARG_NB_USER" \
    USER_GID=$ARG_NB_GID \
    XDG_CACHE_HOME="/home/$ARG_NB_USER/.cache/" \
    XDG_RUNTIME_DIR="/tmp" \
    DISPLAY=":1" \
    TERM="xterm" \
...
# Create user
RUN \
    groupadd -f -g $NB_GID users || true && \
...
FROM workspace-base AS workspace-runtimes
...
FROM workspace-runtimes AS workspace-tools
...
FROM workspace-tools AS workspace-ml
...
FROM workspace-ml AS workspace-jupyter
...
FROM workspace-jupyter AS workspace-full
...
RUN \
    fix-permissions.sh $HOME && \
    fix-permissions.sh $RESOURCES_PATH && \
    fix-permissions.sh $WORKSPACE_HOME && \
    fix-permissions.sh $SSL_RESOURCES_PATH && \
    # Allow user to write to necessary system folders
    # Supervisor
    mkdir -p /var/log/supervisor && chown -R $NB_USER:$NB_GID /var/log/supervisor && \
    mkdir -p /var/run/ && chown -R $NB_USER:$NB_GID /var/run/ && \
    chown -R $NB_USER:$NB_GID /etc/supervisor && \
    # Nginx
    mkdir -p /var/log/nginx && chown -R $NB_USER:$NB_GID /var/log/nginx && \
    mkdir -p /usr/local/openresty/nginx/conf && chown -R $NB_USER:$NB_GID /usr/local/openresty/nginx/conf && \
    chown -R $NB_USER:$NB_GID /etc/nginx && \
    # SSH
    chown -R $NB_USER:$NB_GID /etc/ssh && \
    # Cron
    chown -R $NB_USER:$NB_GID /etc/cron.d
